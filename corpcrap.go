...
func main() {
	...
	statement := adverbs[rand.Intn(len(adverbs))] + " " + verbs[rand.Intn(len(verbs))] + " " + adjectives[rand.Intn(len(adjectives))] + " " + nouns[rand.Intn(len(nouns))]
	formatted := string(bytes.ToUpper([]byte{statement[0]})) + statement[1:]
